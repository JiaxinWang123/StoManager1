...
"")
        ...
        self.pushButton_3.clicked.connect(self.Analysis_S)        
...
"")
    ...
    def retranslateUi(self, StoManager1):
        ...
        StoManager1.setWindowTitle(_translate("StoManager1", "StoManager1.v.0504.23.Seg-x.For_Hardwoods"))
        ...
        if bool(glob.glob(self.input_image_path_ + "/" + '*jpg')) is True:
            ...
            for img_path in images_path:
                ...
                image_name = {"Labels": labels, "Width_(pixels)": widths, "Height_(pixels)": heights,
                            ...
                            "Whole_stomata_density": len(Whole_stomata_density)/(height*width/((pixle)*10.0)**2)}
    ...
    def GuardCell(self):
        ...
        if bool(glob.glob(self.folder + "/" + '*jpg')) is True:
            ...
            for filename in os.listdir(self.folder):
                ...
                if os.path.exists(str(os.path.join(self.output_folder, "Predict_output"))) is False:
                    output_path = os.makedirs(str(os.path.join(self.output_folder, "Predict_output")))
                else:
                    output_path = os.path.join(self.output_folder, "Predict_output")
                    pass
                ...
                if os.path.exists(str(os.path.join(str(output_path), "Output_csv"))) is False:
                    new_path = os.makedirs(str(os.path.join(str(output_path), "Output_csv")))
                else:
                    new_path = os.path.join(str(output_path), "Output_csv")
                    pass
                ...
                def StomGuardCell(imgPath, p):
                    ...
                    results = model.predict(images, save=True, save_txt = True, imgsz=640, conf=self.confidence,  
                                            ...
                                            max_det=500) # implement prediting and pass the results to results
                    ...
                    for i in range(len(results[0].masks.xyn)): # build a for loop to extract segment elements
                        ...
                        box_w = box.xywh.tolist()[0][2] # the bounding box with xywh format, (N, 4)
                        box_h = box.xywh.tolist()[0][3] # the bounding box with xywh format, (N, 4)
                        ...
                        edge_length = (Point(x1[0], y1[0]).distance(Point(x1[1], y1[1])), Point(x1[1], y1[1]).distance(Point(x1[2], y1[2])))
                        # get length of polygon as the longest edge of the bounding box
                        length = int(max(edge_length))
                        # get width of polygon as the shortest edge of the bounding box
                        width = int(min(edge_length))  
                        seg_area = int(PolyArea(x,y))
